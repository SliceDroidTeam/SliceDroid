...
done < config_files/kprobes.list
...
done < config_files/kprobes_conditional.list
...
while IFS=',' read -r mode name; do
    ...
    all_pids+=" $pids"
done < config_files/pid_targets.list
...
done < config_files/events_to_filter.list

# Enable events
while read -r event; do
    echo 1 > "/sys/kernel/tracing/$event/enable"
done < config_files/events_to_enable.list
