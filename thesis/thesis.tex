...
\begin{enumerate}[label=(\roman*.)]
  ...
  \item \textbf{Provenance-system perspectives.}
        Enterprise frameworks such as CamFlow, Hi-Fi and Spade record lightweight syscall provenance for APT detection on desktop systems; Android’s semantic gap hampers their direct reuse, but Binder-aware kernel tracing (e.g., SliceDroid) can supply the missing context and extend host-based detection to mobile :contentReference[oaicite:2]{index=2}.

  \item \textbf{Built-in transparency mechanisms.}
        Since Android 12, status-bar indicators flag camera/mic use by third-party apps, yet they exclude system services and can be bypassed via permission abuse, overlays or framework bugs.  Kernel-level device-file tracing, as done by SliceDroid, closes these blind spots :contentReference[oaicite:3]{index=3}.
\end{enumerate}
...
The system’s user interface has been carefully designed to support intuitive exploration and interpretation of complex kernel-level traces. Users begin by uploading raw \texttt{.trace} files through an integrated upload module, which provides real-time progress tracking and background processing to ensure responsiveness during large file handling. Once ingested, traces are visualised in an interactive event timeline that supports category-based filtering, zoom controls, and detailed tooltips to inspect individual read, write, ioctl, Binder, and network events.

Security analysis features are embedded throughout the dashboard, enabling automatic detection of potential privilege escalation attempts, debugging activities, and other suspicious patterns. Risk levels are indicated with contextual badges and accompanied by targeted security recommendations to aid quick decision-making.

For network behaviour, dedicated views illustrate TCP and UDP connections, protocol distributions, and communication intensity over time, complemented by flow diagrams that clarify external and internal data exchanges. Process analysis tools reconstruct process genealogy, track fork and exec chains, and highlight abnormal execution sequences that may suggest malicious behaviour.

The analytics suite extends these capabilities with configurable parameters for windowed analysis, statistical summaries, and charts illustrating the distribution of syscall categories and device usage. Users can apply fine-grained filters based on process IDs, devices, or syscall classes and export selected results in JSON or CSV formats for offline inspection or reporting.

All visual elements leverage D3.js for dynamic, interactive charts, while the layout employs a modern, responsive design built with Bootstrap. The dashboard features a collapsible sidebar for efficient navigation, integrated help documentation, real-time system status indicators, and health checks to ensure continuous, error-free operation.
