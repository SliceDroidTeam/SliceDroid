...
% --------------------------------------------------
\begin{abstract}
This thesis introduces a hybrid security analysis methodology for generating detailed behavioral profiles of popular mobile messaging applications by combining static inspection with custom kernel-level dynamic tracing. Specifically, the dynamic tracing extends the SliceDroid framework, enabling granular monitoring of system events, inter-process communication, network interactions, and other kernel-level activities.

To demonstrate its effectiveness, comprehensive static and dynamic analyses were performed on three widely used messaging apps: Signal, Telegram, and Meta Messenger. Static inspection, using tools such as MobSF, APKID, and Androguard, revealed differences in manifest configurations, permissions, cryptographic implementations, and embedded third-party components.

For dynamic evaluation, the extended SliceDroid collected precise runtime data under realistic usage scenarios, capturing file system operations, IPC calls, network flows, and resource consumption. The combined analyses uncovered significant behavioral differences: Messenger exhibited the highest runtime complexity and heavy IPC usage, and notably accessed and synchronized user contacts even when explicit contact permissions were revoked—a hidden privacy risk undetectable through static analysis alone. Telegram showed an extensive network footprint consistent with its more cloud-centric model, while Signal maintained the leanest runtime profile, validating its privacy-oriented design.
...
\begin{itemize}
\item \textit{Package Mapping Scope:} The current implementation of the package extraction and mapping algorithm accurately enumerates user-installed APKs but omits system applications such as Chrome, since the underlying ADB commands and permission models treat these as protected packages. A more advanced static-dynamic correlation would require additional privilege escalation or alternative enumeration techniques.
\item \textit{Semantic Granularity:} The tracing layer records low-level kernel events (system calls, Binder transactions) which do not map one-to-one to user-level actions. Bridging this semantic gap demands contextual heuristics or machine learning models, which were beyond the practical scope of this thesis.
\item \textit{Trace Volume and Overhead:} Continuous tracing produces large raw data with redundant or low-entropy segments. While filters and time limits help reduce storage size, they also risk losing relevant context or rare events.
\item \textit{Privilege Requirements:} Complete access to advanced tracepoints and instrumentation (e.g., kprobes, ftrace, tracefs) necessitates root-level permissions, which limits repeatability on production devices.
\item \textit{Attribution Noise:} Shared system services, such as the Android \texttt{servicemanager}, introduce cross-process noise in IPC traces, leading to occasional event misattribution between applications. Extended observation windows help mitigate this, but some uncertainty remains.
\item \textit{Network and Metadata Depth:} The network-layer analysis relied primarily on high-level event capture; deeper inspection of packet contents, handshake flows, or protocol semantics (e.g., with \texttt{tcpdump} or deep packet inspection tools) was not performed, leaving room for future refinement.
\item \textit{Ground Truth Approximation:} Dynamic labels were inferred through controlled user interaction and corresponding system activity. While indicative, this approach may overlook hidden logic or runtime conditions that arise only under specific user contexts.
\item \textit{Static Analysis Coverage:} The static inspection focused on declared configurations, manifest flags, and embedded resources. Obfuscated or dynamically loaded code, which could conceal additional tracking modules or runtime logic, remains out of reach for conventional static scanners and was not exhaustively analysed here.
\end{itemize}

Taken together, these limitations do not undermine the validity of the core findings but should be considered when interpreting the results and generalising them to broader or more complex deployment scenarios. Future work addressing these constraints—through increased automation, expanded hardware coverage, deeper network capture, and enhanced semantic mapping—would further strengthen the robustness and applicability of the conclusions drawn.
...
\begin{itemize}
    ...
    \item Identifies critical privacy-related insights, including permission discrepancies and hidden runtime behaviors not discoverable through static analysis alone.
\end{itemize}
...
In contrast, the \textit{Behavior Timeline} (Figure~\ref{fig:messenger-none-behav}) reveals a more significant finding. Despite explicit permission restrictions, Messenger continues to access the \texttt{contacts} category repeatedly, along with TCP connections. This persistent contact access suggests that parts of the contact handling logic remain active even when permission checks should prevent it.
...
To verify this behaviour, we repeated the same scenario ten times under identical device conditions. The unexpected contacts access occurred in nine out of ten runs, confirming that this is not random but a consistent pattern. To clarify the cause, we contacted both Meta (the app provider) and Google (responsible for Android permission enforcement). Based on the responses, this was confirmed as unintended permission misuse, attributed to residual background processes that bypass the expected restriction mechanisms.

The \textit{Communication Flow} diagram (Figure~\ref{fig:messenger-none-flow}) shows a relatively simple runtime graph, with a few worker PIDs attached to a central dispatcher. This reflects the lighter process orchestration typical of Messenger when more advanced features are disabled, yet it still includes modules responsible for background syncs.
...
Overall, while the core event sequence stays consistent with other scenarios, the persistent contacts access under restricted mode highlights a privacy concern that requires closer examination and mitigation.
...
\subsection{Cross-Application Comparative Analysis and Statistical Validation}
...
The results clearly show that static analysis alone is insufficient. Kernel-level traces uncovered runtime behaviours inconsistent with manifest declarations — for example, Messenger continued to access contacts metadata even under restricted permissions. Dynamic analysis also revealed runtime characteristics such as hidden background sync operations and large-scale prefetching that static tools cannot detect. Thus, combining both static and dynamic methods is necessary to comprehensively identify privacy-relevant behaviours.
...
While this thesis provides a detailed static and dynamic security assessment of popular mobile messaging applications, several opportunities remain for enhancing the depth, breadth, and generalisability of the findings. A notable avenue for improvement involves expanding the runtime dataset by extending the duration of dynamic traces and diversifying the user interaction scenarios to include more complex features such as group video calls or large file transfers—operations that could introduce additional system calls or unexpected privacy-relevant behaviour, including UDP-based traffic which was not fully exercised under the chosen workloads. Moreover, refining the package-mapping algorithm to detect both user-installed and system applications would allow future studies to profile native services and bundled browsers like Chrome, whose behaviour is highly relevant for holistic mobile security evaluations.

Methodologically, complementing kernel-level tracing with packet-level network captures—using tools such as \texttt{tcpdump} or Wireshark—would enable fine-grained inspection of encrypted and clear-text flows, enriching the understanding of metadata leakage and verifying the consistency between declared transport security policies and actual network interactions. Similarly, integrating metadata enrichment scripts directly into the tracing pipeline could automate the labelling of behaviour patterns, reducing manual annotation effort and improving trace interpretability.

Finally, scaling the experimental infrastructure to include multiple device types, Android versions, and more varied hardware profiles would strengthen the external validity of the results. Deploying lightweight, automated tracing frameworks with minimal performance overhead could facilitate longer and more diverse measurement campaigns without overwhelming local storage or compute resources. Collectively, these improvements would deepen the insights into how design choices and runtime conditions affect the privacy posture of mobile messaging applications, and would lay a stronger empirical foundation for future research, policy recommendations, and secure application design practices.
...
Building upon the limitations and lessons identified in this study, several promising directions emerge for future research in mobile application security and privacy evaluation. First, extending the dynamic analysis to encompass longer sessions and more diverse feature sets—such as group video calls, file sharing, and background push services—could uncover subtler runtime behaviours and further stress-test each application’s privacy guarantees. Integrating packet-level network captures alongside kernel traces would provide complementary visibility into encrypted traffic patterns and metadata flows, offering a richer basis for detecting potential privacy leakages or transport-layer inconsistencies.

Methodologically, refining the package-mapping and attribution mechanisms to include system-level processes and privileged services would broaden the scope beyond user applications, enabling a more holistic view of the mobile ecosystem’s security posture. Automated annotation tools, powered by pattern recognition or machine learning, could enhance the semantic labelling of trace data, bridging the gap between low-level syscall logs and high-level user actions.

Additionally, replicating the analysis across a wider variety of hardware platforms, Android versions, and regional app variants would strengthen the generalisability of the conclusions and help identify device-specific or locale-specific privacy risks. Finally, expanding the comparative scope to include less popular or niche messaging apps—especially those marketed on privacy claims—could provide valuable insight into whether smaller vendors maintain the same level of hardening and resource discipline as their mainstream counterparts. Collectively, these avenues would deepen the empirical foundation for evidence-based privacy assurance, informed policy development, and secure-by-design mobile application engineering.
