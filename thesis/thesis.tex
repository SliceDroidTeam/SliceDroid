...
The dynamic analysis was systematically conducted on three widely used messaging
applications—\textit{Messenger}, \textit{Signal}, and \textit{Telegram}—capturing
their runtime behaviour under the exact usage scenarios previously defined and
executed in Section~4.3.4 \textit{Workload Design and Usage Scenarios}.
As described in that section, each application was exercised in a controlled
environment for approximately $30 \pm 1$ seconds per run, under four distinct
operational modes: running in the background with full operational permissions
(\texttt{full\_background}), actively used in the foreground with full permissions
(\texttt{full\_foreground}), background execution with minimal resource activity
...
application, scenario, and recording duration, providing full traceability and alignment with the workload structure.
Observed file sizes demonstrate an expected trend: foreground traces under full permissions exhibit significantly larger
footprints due to richer event flows from user interactions and multimedia activities, while background and
constrained scenarios yield notably smaller trace volumes, reflecting lower runtime resource engagement.
For instance, \textit{Messenger} generated a 117~KB trace in the full foreground scenario compared to a 16~KB
trace in the none background condition. This structured dataset reflects the defined workload scenarios
and provides a reliable basis for comparing application behaviour, resource usage, and runtime characteristics
