...
For dynamic evaluation, the extended SliceDroid collected precise runtime data under realistic usage scenarios, capturing file system operations, IPC calls and network flows. The combined analyses uncovered significant behavioral differences: Messenger exhibited the highest runtime complexity and heavy IPC usage, and notably, it was discovered that Messenger continues to access user contacts even after permission is revoked. This is achieved through a persistent sync adapter state in Google Play Services, revealing a fundamental flaw in the Android permission model that static analysis alone could not uncover.
...
To identify the source of these operations, we performed a detailed Binder Inter-Process Communication (IPC) analysis. Tracing revealed that the access originates not from Messenger itself, but from a Content Provider process (PID 27488, UID 10064) activated by a Binder worker thread. A representative excerpt from the kernel trace is shown in Listing~\ref{lst:binder-trace}.

% Listing starts on a new page as requested
\newpage
...
This systematic analysis allowed us to reconstruct the complete information flow, which circumvents the user's explicit denial of permission. The violation occurs via a three-stage process involving Messenger, Google Play Services, and the Android Content Provider, as illustrated in Figure~\ref{fig:messenger-indirect-flow}. The investigation concluded that this vulnerability stems from a \textbf{persistent sync adapter state} that survives permission revocation.
...
\begin{figure}[H]
    ...
    \node[db, below=of provider] (db) {\texttt{contacts2.db}};
...
The timeline of this root cause is as follows:
\begin{enumerate}
...
This design flaw in the Android permission lifecycle is illustrated in Figure~\ref{fig:sync-persistence}. This vulnerability has significant privacy implications, creating a "control illusion" where users believe their actions have secured their data, when in reality, access continues unabated. It represents a systemic risk, as it likely affects any application using this common Google Play Services sync pattern.
...
\begin{figure}[H]
    ...
    \node[
        ...
        rounded corners,
        inner sep=8pt,
        label={[text=red!80!black, font=\sffamily\bfseries]right:DESIGN FLAW}
    ] (flaw_box) {};
...
\end{figure}
Our findings reveal a critical flaw in Android's permission architecture: the model is incomplete because it fails to manage the lifecycle of persistent states, such as sync registrations, upon permission changes. This allows privileged system services to be unknowingly exploited as privacy bypasses, undermining the foundation of informed consent in the mobile ecosystem.
...
The most critical finding is the discovery that Messenger accesses the contacts2.db database even when its permission is denied. This exemplifies a critical "permission gap" in Android. Our kernel-level analysis successfully traced the indirect information flow: Messenger initiates an API call to Google Play Services, which in turn triggers a persistent SyncAdapter that accesses the contacts Content Provider with system-level privileges. This confirms an interaction that completely bypasses the user-facing permission model.
...
Among the most conclusive findings was the discovery of a persistent privacy violation by Messenger, which continues to access contact-related resources even after its permission is explicitly revoked. Our analysis provides definitive proof that this occurs due to a persistent sync adapter state maintained by Google Play Services, which acts as a proxy. The precise origin of this access is no longer inconclusive; it is a direct consequence of a design flaw in the Android permission model where revoking a permission does not clear associated sync states, creating a hidden data access channel.
