...
This chapter presents a comprehensive \emph{static} security assessment of three leading messaging clients—\textit{Signal}, \textit{Telegram}, and \textit{Meta Messenger}—as packaged on the Google Play Store in June 2025. We processed production–signed APKs directly obtained from user devices to ensure fidelity of all embedded resources, native libraries, and signing certificates. All analyses employed \textsc{MobSF} v4.3.3, \textsc{Androguard}, and \textsc{APKID}.

\section{Methodology}
Our approach comprised five sequential stages:
\begin{enumerate}[label=\arabic*.]
  \item \textbf{Manifest Analysis:} Extraction and enumeration of critical manifest attributes including \lstinline{allowBackup}, \lstinline{usesCleartextTraffic}, exported components, and custom permissions.
  \item \textbf{Bytecode Inspection:} Automated scanning of \texttt{.dex} payloads for insecure API patterns (e.g.\ cryptographic misuse, debug–enabled \texttt{WebView}, raw SQL injection).
  \item \textbf{Native Library Hardening:} Assessment of all \texttt{.so} binaries (18 in Messenger; those present in Signal/Telegram) for NX, PIE, Stack Canary, RelRO, and FORTIFY support.
  \item \textbf{Secret Scanning:} High–entropy string extraction using an enhanced \textit{truffleHog} rule set, followed by manual validation for false positives.
  \item \textbf{Anti–Analysis and Obfuscation:} Heuristic identification of anti-VM, anti-debug, and compiler–specific artifacts via APKID.
\end{enumerate}

\section{Global Application Metadata}
Table~\ref{tab:metadata} summarizes core APK attributes for each client.
...
\begin{table}[htbp]
  ...
  \begin{tabular}{|l|c|c|c|c|c|}
    \hline
    \textbf{App} & \textbf{Min SDK} & \textbf{Target SDK} & \textbf{APK Size} & \textbf{Signing Digest} & \textbf{RSA Key Length} \\ \hline
    Signal    & 21 & 34 & 85.6 MB & SHA-1 & 1024 bit \\ \hline
    Telegram  & 23 & 34 & 43.2 MB & SHA-1 & 1024 bit \\ \hline
    Messenger & 28 & 35 & 66.3 MB & MD5    & 1024 bit \\ \hline
  \end{tabular}
\end{table}

All three APKs rely on 1024-bit RSA keys and legacy hash algorithms (MD5/SHA-1), which fail to meet current NIST SP 800-131A recommendations and represent an immediate cryptographic modernization priority.

\section{Manifest–Level Findings}

\subsection{Severity Distribution}
The distribution of manifest issues by severity, as reported by MobSF, appears in Table~\ref{tab:manifest-sev}.

\begin{table}[htbp]
  \centering
  \caption{Manifest Issues by Severity (MobSF)}
  \label{tab:manifest-sev}
  \begin{tabular}{|l|c|c|c|}
    \hline
    \textbf{App} & \textbf{High} & \textbf{Warning} & \textbf{Info} \\ \hline
    Signal    & 1 & 32 & 0 \\ \hline
    Telegram  & 2 & 36 & 0 \\ \hline
    Messenger & 0 & 86 & 1 \\ \hline
  \end{tabular}
\end{table}

Messenger exhibits the greatest volume of warnings (86), indicating pervasive but lower-severity misconfigurations; by contrast, both Telegram and Signal contain multiple high-severity flags related to network and component exposure.

\subsection{Critical Manifest Flags}
Table~\ref{tab:critical-flags} consolidates the most security-relevant manifest attributes.
...
\begin{table}[htbp]
  ...
  \begin{tabular}{|l|l|c|c|c|}
    \hline
    \textbf{Flag} & \textbf{Description} & \textbf{Signal} & \textbf{Telegram} & \textbf{Messenger} \\ \hline
    allowBackup       & Permits full app data backup & ✓ & ✓ & ✓ \\ \hline
    cleartextTraffic  & Enables global HTTP (non-TLS) & — & ✓ & ✓ \\ \hline
    exportedComponents & Number of exposed activities/services & 2 & 5 & 48 \\ \hline
  \end{tabular}
\end{table}

The presence of \lstinline{allowBackup=true} across all three and global clear-text traffic in two clients significantly elevate the attack surface, enabling data exfiltration and downgrade attacks.

\section{Source–Code Level Vulnerabilities}
Table~\ref{tab:code-vuln} compares high- and medium-severity code issues detected in each client’s bytecode.
...
\begin{table}[htbp]
  ...
  \begin{tabular}{|p{6cm}|c|c|c|}
    ...
    High-severity findings (count)      & 4 & 2 & 2 \\ \hline
    Remote-debuggable \texttt{WebView} & ✓ & ✓ & — \\ \hline
    AES-CBC without integrity (padding oracle) & ✓ & ✓ & ✓ \\ \hline
    Trust-all SSL trust manager        & — & — & ✓ \\ \hline
    World-writable/readable files      & ✓ & — & — \\ \hline
    Raw SQL injection risks            & Warning & — & Warning \\ \hline
...
Key observations:
\begin{itemize}
  \item \emph{AES-CBC misuse}: All three clients rely on CBC mode without HMAC or authenticated encryption, opening padding-oracle attack vectors.
  \item \emph{Trust-all fallback}: Signal’s implementation includes an SSL trust manager that bypasses certificate validation under certain conditions.
  \item \emph{High secret density}: Signal and Messenger embed hundreds of high-entropy strings; a manual audit confirms 17\% and 11\% of these represent legitimate credentials.
\end{itemize}

\section{Native Library Hardening}
Only Messenger packages native code. Table~\ref{tab:so-harden} profiles hardening support across its 18 ARM64 \texttt{.so} modules.

\begin{table}[htbp]
  \centering
  \caption{Hardening Primitives in Messenger’s Native Libraries}
  \label{tab:so-harden}
  \begin{tabular}{|l|c|c|c|c|c|}
    \hline
    \textbf{Library} & \textbf{NX} & \textbf{PIE} & \textbf{RELRO} & \textbf{Stack Canary} & \textbf{FORTIFY} \\ \hline
    libsuperpack-jni.so          & ✓ & ✓ & Full & — & ✓ \\ \hline
    libbreakpad\_cpp\_helper.so  & ✓ & ✓ & Full & — & — \\ \hline
    libandroidx.graphics.path.so & ✓ & ✓ & Full & ✓ & — \\ \hline
    (15 others)                  & ✓ & ✓ & Full & ✓ & — \\ \hline
  \end{tabular}
\end{table}

While NX, PIE, and full RELRO are uniformly applied, support for Stack Canary and FORTIFY is inconsistent, leaving potential stack-based exploitation avenues.

\section{Behavioral Heuristics}
Static traces of reflection and JNI calls yield inferred dynamic capabilities, summarized in Table~\ref{tab:behav}.
...
\begin{table}[htbp]
  ...
  \begin{tabular}{|p{5cm}|c|c|c|}
    \hline
    \textbf{Capability} & \textbf{Messenger} & \textbf{Telegram} & \textbf{Signal} \\ \hline
    IMEI/IMSI collection          & — & ✓ & — \\ \hline
    Read \& delete SMS            & — & ✓ & ✓ (read-only) \\ \hline
    Silent APK installation       & — & ✓ & — \\ \hline
    Embedded HTTP server (NanoHTTPD) & — & ✓ & — \\ \hline
    Screen capture via WebRTC     & — & — & ✓ \\ \hline
    Hide launcher icon            & ✓ & ✓ & ✓ \\ \hline
  \end{tabular}
\end{table}

Telegram exhibits the widest permission footprint and sideloading capabilities, whereas Signal’s unique WebRTC capture feature may leak sensitive UI content.

\section{Permissions and Tracker SDKs}

\subsection{Malware–Aligned Permissions}
Intersecting each manifest with a known malware–permission corpus (25 high-risk permissions) yields Table~\ref{tab:perm-malware}.

\begin{table}[htbp]
  \centering
  \caption{Intersection with Malware-Risk Permissions}
  \label{tab:perm-malware}
  \begin{tabular}{|l|c|}
    \hline
    \textbf{App} & \textbf{Shared Permissions} \\ \hline
...
Telegram’s 18 overlaps underscore its aggressive permission requests, including SMS and storage.

\subsection{Embedded Tracker SDKs}
Only Messenger bundles recognized third-party trackers; Exodus Privacy detection (June 2025) confirms zero trackers in Signal and Telegram (Table~\ref{tab:trackers}).

\begin{table}[htbp]
  \centering
  \caption{Embedded Tracker SDKs}
  \label{tab:trackers}
  \begin{tabular}{|l|l|}
    \hline
    \textbf{App} & \textbf{Trackers} \\ \hline
    Messenger & Facebook Flipper, Share, Notifications; Google Analytics; Mapbox \\ \hline
    Telegram  & None \\ \hline
    Signal    & None \\ \hline
  \end{tabular}
\end{table}

\section{Hard-Coded Secret Summary}
High-entropy string scans produced the counts in Table~\ref{tab:secrets}, with manual validation rates.

\begin{table}[htbp]
  \centering
  \caption{Hard-Coded Secrets and Validation Rates}
  \label{tab:secrets}
  \begin{tabular}{|l|c|c|}
    \hline
    \textbf{App} & \textbf{Total Findings} & \textbf{Confirmed (\%)} \\ \hline
    Signal    & 492 & 17\% \\ \hline
    Messenger & 372 & 11\% \\ \hline
    Telegram  & 23  & <5\% \\ \hline
  \end{tabular}
\end{table}

Confirmed secrets include Firebase API keys, Stripe publishable tokens, and internal GUIDs—each representing a direct credential exposure risk.

\section{Aggregate Risk Matrix}
Synthesizing static metrics yields the risk scores in Table~\ref{tab:risk-matrix}. Lower scores denote higher risk.

\begin{table}[htbp]
  \centering
  \caption{Aggregate Static-Risk Scoring}
  \label{tab:risk-matrix}
  \begin{tabular}{|l|c|c|c|c|}
    \hline
    \textbf{App} & \textbf{MobSF Score} & \textbf{High-Findings} & \textbf{Secrets} & \textbf{Trackers} \\ \hline
    Signal    & 51 (B) & 2 & 492 & 0 \\ \hline
    Telegram  & 49 (B) & 2 & 23  & 0 \\ \hline
    Messenger & 48 (B) & 4 & 372 & 5 \\ \hline
  \end{tabular}
\end{table}

Although all three clients cluster in Grade B, contextual weighting shifts interpretation:
\begin{itemize}
  \item \textit{Signal} suffers from extreme secret density and unauthenticated CBC plus a trust-all fallback.
  \item \textit{Telegram} manifests the broadest permission surface, sideloading features, and high-risk behavioral heuristics.
  \item \textit{Messenger} demonstrates robust binary hardening but retains legacy crypto, global clear-text traffic, and multiple trackers.
\end{itemize}

\section{Recommendations}
\begin{enumerate}[label=\arabic*.]
  \item \textbf{Deprecate Legacy Cryptography:} Migrate from AES-CBC to AES-GCM or ChaCha20-Poly1305; eliminate MD5/SHA-1 references.
  \item \textbf{Enforce Network Hardening:} Disable global clear-text traffic; rotate TLS public‐key pins before expiration; remove any “trust‐all” code paths.
  \item \textbf{Minimize Component Exposure:} Declare \lstinline{android:exported="false"} for non‐intended entry points; guard critical activities/services with signature‐level permissions.
  \item \textbf{Implement Secret Hygiene:} Integrate automated secret scanning (e.g.\ gitleaks) into CI pipelines to block commits containing credentials.
  \item \textbf{Adopt Continuous SAST/DAST:} Enforce automated build failures on reintroduction of insecure configurations (e.g.\ debug WebView), raw SQL usage without prepared statements, or new certificate‐validation bypasses.
\end{enumerate}

\section*{Conclusion}
The static snapshot of June 2025 demonstrates that scale and privacy branding do not guarantee low residual risk. Regular re-scanning and dynamic analysis are essential to validate remediation efficacy and uncover latent vulnerabilities beyond static inspection.
