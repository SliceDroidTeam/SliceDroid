...
\cite{arxiv2020metadata,obermeier2018signal,wired2023signalhack}. Messaging apps may still leak
...
\subsubsection*{(v) The SliceDroid as a Foundational Methodology}
To bridge the significant \textbf{semantic gap} between high-level application behavior and low-level kernel events, our methodology is based on and extends the functionality of \textbf{SliceDroid}~\cite{Alexopoulos2025SliceDroid}. This framework provides a foundation for reconstructing I/O behaviors from kernel traces through Inter-Process Communication (\texttt{IPC}) slicing.

\medskip
\noindent\textbf{Synthesis.}
In summary, kernel-level tracing provides a powerful, tamper-resistant vantage point for analyzing system behavior. However, the literature consistently grapples with four primary challenges: bridging the \textbf{semantic gap} between kernel events and application logic, handling opaque data, overcoming strict privilege requirements, and managing the immense volume of trace data. While various approaches have made progress, the \textbf{SliceDroid} framework offers a particularly compelling solution focused on bridging the semantic gap transparently and efficiently. It is for this reason that we have selected its methodology as the foundation for our own work, as detailed in the following section.
...
Early work focused on residual data stored locally. Anglano’s systematic analysis of WhatsApp on Android~4.x recovered chat databases, media thumbnails, and contact lists even after user-initiated deletions~\cite{anglano2015whatsapp}. Subsequent studies extended the corpus to Telegram, Signal, and Viber, showing that cached profile photos, push-notification logs, and SQLite WAL files can reveal conversation partners and group identifiers~\cite{moltchanov2018telegram, obermeier2018signal}. While modern OS versions encrypt local backups, notification side-channels (e.g., Firebase tokens) remain an open avenue for investigation.

\subsubsection*{(ii) Network-Level Traffic Analysis}

Analyzing mobile application traffic at the network level is a key method for detecting privacy risks that static or platform-level analysis may miss. By inspecting real-time transmissions, researchers can uncover sensitive data leaks, including personally identifiable information (PII) and contacts with third-party domains.

Egele et al.~\cite{egele2011pios} proposed \textit{PiOS}, a framework that combines static and dynamic analysis to detect privacy leaks in iOS apps, revealing unauthorized transmission of device identifiers and location data. In the Android space, Nayam et al.~\cite{nayam2016analysis} examined the network behavior of popular apps and found frequent communication with third-party servers, particularly for ads and analytics, often without user awareness.

These studies show that network-level analysis is essential for identifying hidden data flows and limitations in permission-based models, and can support privacy auditing and malicious app detection.

\subsubsection*{(iii) Metadata Policies and Compliance Audits}
Research into the privacy of messaging applications increasingly scrutinizes metadata policies, revealing significant disparities despite the widespread adoption of end-to-end encryption (E2EE) \cite{reardon2019sok}. While E2EE protects message content, applications like WhatsApp, Telegram, and Signal vary greatly in the type and volume of metadata they collect \cite{nikitin2021analysis, eff2018scorecard}, which can include timestamps, contact lists, IP addresses, and user location. Studies consistently highlight that this metadata can be highly revealing, enabling the inference of social graphs, user behavior, and sensitive personal details. Compliance audits, intended to verify adherence to stated privacy policies (e.g., GDPR requirements), are often challenging and opaque \cite{datta2021auditing}. Independent audits are rare, and companies typically rely on internal reviews or commissioned reports whose methodologies are not always public, creating a significant trust gap \cite{angwin2021propublica}. Consequently, users must often rely on the provider's claims about minimizing metadata collection \cite{whatsapp2025policy, telegram2025policy} rather than on verifiable proof. Academic and technical analyses frequently find that even when policies are respected, architectural choices can lead to unintentional metadata leakage \cite{cohngordon2017formal}, underscoring the need for more rigorous auditing and privacy-preserving designs like Signal's Sealed Sender \cite{marlinspike2018sealed} to ensure robust user privacy.
...
Android, as the dominant mobile operating system, provides the primary distribution platform for these apps through the Google Play Store. According to public data, Facebook Messenger has surpassed 5 billion downloads, Telegram exceeds 1.2 billion downloads, and Signal has more than 100 million installs \cite{StatistaMessenger, StatistaSignal}. While usage varies by region, these numbers highlight the ubiquity and market penetration of messaging applications on Android devices.
...
Analysis of the collected traces and data reveals that this indirect access constitutes a three-stage bypass of the Android permission model. This bypass involves the interaction between three key components: the Messenger application, Google Play Services, and the Android Content Provider. This process, as documented by our findings, is illustrated in the diagram in Figure~\ref{fig:messenger-indirect-flow}.
\begin{figure}[H]
    \centering
    \begin{tikzpicture}[
...
More specifically, the analysis allows us to ascertain the three distinct stages that constitute this bypass. These stages, which reveal the flow of information from Messenger’s initial call to the final database access, are detailed below:
...
This information flow was corroborated by Slicedroid, which confirmed the communication between the three implicated processes.


Our central hypothesis is that the vulnerability's core lies in a persistent sync adapter state that survives permission revocation. The timeline below, also illustrated in Figure~\ref{fig:sync-persistence}, explains this critical flaw:
...
The initial findings of this study highlight a potentially critical privacy issue that requires further, more extensive research and analysis to produce fully robust conclusions. As it stands, the current analysis serves as a proof-of-concept, demonstrating that the vulnerability likely exists under a specific set of conditions. To move from this initial observation to a generalizable conclusion, a broader and more systematic investigation is necessary.
\chapter{Discussion}
