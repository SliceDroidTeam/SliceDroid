 ...
 * This function is called by data-loader.js
 */
function renderTimeline() {
    // Use modern TimelineChart if available
    if (typeof TimelineChart !== 'undefined' && window.timelineData) {
        try {
            console.log('Rendering timeline with modern TimelineChart');
            const timelineChart = new TimelineChart('timeline-container', {
                title: 'Event Timeline',
                showBrush: true,
                showZoom: true
            });
            timelineChart.setData(window.timelineData);
            return;
        } catch (error) {
            console.warn('Failed to use modern TimelineChart, falling back to legacy:', error);
        }
    }
    
    // Legacy timeline rendering
    console.log('Rendering timeline with legacy method');
    ...
    if (!window.timelineData || window.timelineData.length === 0) {
        container.innerHTML = createEmptyState('No timeline data available');
        return;
    }
    
    // Simple timeline implementation for backward compatibility
    container.innerHTML = `
        <div class="timeline-legacy">
            <h6>Timeline Data Loaded</h6>
            <p class="text-muted">${window.timelineData.length} events loaded</p>
            <div class="alert alert-info">
                <i class="fas fa-info-circle me-2"></i>
                Timeline visualization will be available once the chart system is fully loaded.
            </div>
        </div>
    `;
}
